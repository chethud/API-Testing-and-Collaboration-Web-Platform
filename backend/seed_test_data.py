...
def seed():
    ...
    requests_data = [
        ...
        },
        {
            ...
            "query_params": {},
            "body": json.dumps({"title": "Test", "body": "Hello", "userId": 1}),
            "body_type": "json",
            "collection_id": col1_id,
        },
        {
    ...
    for r in requests_data:
        existing_req = conn.execute(
            "SELECT id FROM api_requests WHERE name = ? AND workspace_id = ?",
            (r["name"], workspace_id),
        ).fetchone()
        if not existing_req:
            cur = conn.execute(
            ...
            conn.execute(
                "INSERT INTO request_history (request_id, snapshot, version, created_by) VALUES (?, ?, 1, ?)",
                (req_id, json.dumps(snap), user_id),
            )
    conn.commit()
    ...
    if not conn.execute("SELECT id FROM mock_endpoints WHERE workspace_id = ? LIMIT 1", (workspace_id,)).fetchone():
        conn.execute(
            ...
            ("/users", "GET", 200, json.dumps([{"id": 1, "name": "Mock User"}]), workspace_id),
        )
        conn.execute(
            "INSERT INTO mock_endpoints (path, method, status_code, response_body, workspace_id) VALUES (?, ?, ?, ?, ?)",
            ("/health", "GET", 200, json.dumps({"status": "ok"}), workspace_id),
        )
        conn.commit()
    ...
    if not conn.execute("SELECT id FROM notifications WHERE user_id = ? LIMIT 1", (user_id,)).fetchone():
        conn.execute(
            ...
            (user_id, "api_failed", "Sample: API request failed", "GET https://example.com → 404", 0),
        )
        conn.execute(
            "INSERT INTO notifications (user_id, type, title, body, read) VALUES (?, ?, ?, ?, ?)",
            (user_id, "request_updated", "Sample: Request updated", "GET posts list was updated", 1),
        )
        conn.commit()
    ...
    print("Workspace and sample requests will appear in the sidebar.")
